...
//                .filter(employee -> Stream.of("Sales And Marketing").anyMatch(s->s.equals(employee.department())))
    ...
    private static void noOfEmployeesInEachDept2(List<Employee> employees) {
    ...
    private static void joinedAfter20152(List<Employee> employees) {
    ...
    private static void highestPaid2(List<Employee> employees) {
        employees.stream().max(Comparator.comparing(Employee::salary)).ifPresentOrElse(employee -> System.out.println("Highest employee salary:"+employee),()-> System.out.println("No employees with salaries"));
    }
    ...
    private static void avarageAgeOfMaleFemale2(List<Employee> employees) {
        System.out.println("Average age of male and female");
        employees.stream().mapToDouble(Employee::age)
                .average()
                .ifPresentOrElse(System.out::println,()->System.out.println("There is no employees in list"));

        System.out.println("Average age of male");
        employees.stream().filter(employee -> employee.gender().equalsIgnoreCase("male")).mapToDouble(Employee::age)
                .average()
                .ifPresentOrElse(System.out::println,()->System.out.println("There is no male employees in list"));
        System.out.println("Average age of female");

        employees.stream().filter(employee -> employee.gender().equalsIgnoreCase("female")).mapToDouble(Employee::age)
                .average()
                .ifPresentOrElse(System.out::println,()->System.out.println("There is no female employees in list"));

        System.out.println("Group By");
        employees.stream().collect(Collectors.groupingBy(Employee::gender,Collectors.averagingInt(Employee::age))).forEach((gender,avgage)-> System.out.println(gender+"::average age =>"+avgage));
    }
    ...
    private static void departmentNames2(List<Employee> employees) {
        employees.stream().map(Employee::department).collect(Collectors.toSet()).forEach(System.out::println);
    }
    ...
    private static void maleAndFemale2(List<Employee> employees) {
        System.out.println("Using sets");
        Set<Employee> male = employees.stream()
                .filter(employee -> employee.gender().equalsIgnoreCase("male"))
                .collect(toSet());
        System.out.println(male);

        Set<Employee> female = employees.stream()
        ...
        System.out.println("Using group by and count");

        employees.stream()
                .collect(Collectors.groupingBy(Employee::gender,Collectors.counting()))
                .forEach((gender,count)-> System.out.println(gender+"=>"+count));

        System.out.println("Using group by and mapping to name");
        employees.stream()
                        .collect(Collectors.groupingBy(Employee::gender,Collectors.mapping(Employee::name,Collectors.toList())))
                .forEach(
                        (gender,list)-> System.out.println(gender + "=>" +list
                        ));
        System.out.println("Using group by and highest salary");

        employees.stream()
                        .collect(Collectors.groupingBy(Employee::gender,Collectors.maxBy(Comparator.comparing(Employee::salary)))).forEach((gender,salary)-> System.out.println(gender + "=>" + salary));
    ...
    private static void maleAndFemale(List<Employee> employees) {
        ...
        for (Map.Entry<String, List<Employee>> group : genderGroups.entrySet()) {
            // Note: Instead we can also use genderGroups.forEach(...)

            System.out.println(group.getKey() + ":");
            group.getValue().stream().forEach(System.out::println);
        }

        genderGroups.forEach((gender,list)->{
            System.out.println(gender +"=>"+ String.join(",",list.stream().map(e->e.name()).collect(Collectors.toList())));
        });

        genderGroups.forEach((gender,list)->{
            System.out.println(gender +"=>"+ list.stream().map(Employee::name).collect(Collectors.joining(",")));
        });
