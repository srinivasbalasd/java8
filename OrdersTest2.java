...
public class OrdersTest2 {
    ...
    public static void main(String[] args) {
        ...
        // Exercise 1 â€” Obtain a list of products belongs to category â€œBooksâ€ with price > 100
        System.out.println("Exercise 1 â€” Obtain a list of products belongs to category â€œBooksâ€ with price > 100");
        products.stream()
                .filter(product -> product.getCategory().equals("Books"))
                .filter(product -> product.getPrice() > 100)
                .forEach(System.out::println);

        // Exercise 2 â€” Obtain a list of order with products belong to category â€œBabyâ€
        System.out.println("\n\nExercise 2 â€” Obtain a list of order with products belong to category â€œBabyâ€");
        orders.stream()
        ...
        // Exercise 3 â€” Obtain a list of product with category = â€œToysâ€ and then apply 10% discount
        System.out.println("\n\nExercise 3 â€” Obtain a list of product with category = â€œToysâ€ and then apply 10% discount");
        products.stream()
        ...
        orders.stream()
                ...
                .filter(order -> order.getOrderDate().isAfter(LocalDate.of(2024, 1, 31)))
                .filter(order -> order.getOrderDate().isBefore(LocalDate.of(2024, 4, 1)))
                .flatMap(order -> order.getProducts().stream())
                .distinct()
                .forEach(System.out::println);
        //Exercise 5 â€” Get the cheapest products of â€œBooksâ€ category
        System.out.println("\n\nExercise 5 â€” Get the cheapest products of â€œBooksâ€ category");
        products.stream()
                .filter(product -> product.getCategory().equals("Books"))
        ...
        // Exercise 7 â€” Get a list of orders which were ordered on 15-Mar-2021, log the order records to the console and then return its product list
        System.out.println("\n\nExercise 7 â€” Get a list of orders which were ordered on 15-Mar-2021, log the order records to the console and then return its product list");
        orders.stream()
                .filter(order -> order.getOrderDate().equals(LocalDate.of(2021, 03, 15)))
                .peek(System.out::println).map(order -> order.getProducts()).forEach(System.out::println);
        // Exercise 8 â€” Calculate total lump sum of all orders placed in Feb 2021
        System.out.println("\n\nExercise 8 â€” Calculate total lump sum of all orders placed in Feb 2021\n");
        System.out.println(orders.stream().filter(order -> order.getOrderDate().isAfter (LocalDate.of(2021, 1, 31)))
                .filter(order -> order.getOrderDate().isBefore(LocalDate.of(2021, 3, 1)))
                .flatMap(order -> order.getProducts().stream().map(p -> p.getPrice()).collect(Collectors.toList()).stream())
                //.mapToDouble(price->price)
                .mapToDouble(price -> price).sum());
        //todo check                      .

        // Exercise 9 â€” Calculate order average payment placed on 14-Mar-2021
        System.out.println("\n\nExercise 9 â€” Calculate order average payment placed on 14-Mar-2021");
        ...
        // Exercise 10 â€” Obtain a collection of statistic figures (i.e. sum, average, max, min, count) for all products of category â€œBooksâ€
        System.out.println("\n\nExercise 10 â€” Obtain a collection of statistic figures (i.e. sum, average, max, min, count) for all products of category â€œBooksâ€");
        DoubleSummaryStatistics summaryStatistics = products.stream().mapToDouble(Product::getPrice)
                .summaryStatistics();
        System.out.printf("%S,,,%S,,,%S,,,%S,,,%S",summaryStatistics.getSum(),summaryStatistics.getAverage(),summaryStatistics.getMax(),summaryStatistics.getMax(),summaryStatistics.getMin(),summaryStatistics.getCount());
        // Exercise 11 â€” Obtain a data map with order id and orderâ€™s product count
        System.out.println("\n\nExercise 11 â€” Obtain a data map with order id and orderâ€™s product count");
        orders.stream().collect(Collectors.toMap(order -> order.getId(),order -> order.getProducts().size())).forEach(((e1,e2)->System.out.println(e1+"::Count ="+e2)));
        //orders.stream().collect(Collectors.groupingBy(Order::getId,  orders.stream().flatMap(order -> order.getProducts().stream()).count());
        System.out.println("2nd solution");
        orders.stream().collect(Collectors.groupingBy(Order::getId,  Collectors.summarizingInt(ords -> ords.getProducts().size()))).forEach(((e1,e2)->System.out.println(e1+"::Count ="+e2)));;

        // Exercise 12 â€” Produce a data map with order records grouped by customer
        System.out.println("\n\nExercise 12 â€” Produce a data map with order records grouped by customer");
        orders.stream().collect(Collectors.groupingBy(order -> order.getCustomer())).forEach((e1,e2)-> System.out.println(e1+"   :   "+e2));
        // Exercise 13 â€” Produce a data map with order record and product total sum
        System.out.println("\n\nExercise 13 â€” Produce a data map with order record and product total sum");
        //orders.stream().collect(Collectors.groupingBy(Order::getId,Collectors.summingDouble(ord -> ord.getProducts().stream().map(Product::getPrice)));
        //todo
        orders.stream().collect(Collectors.toMap(Order::getId,order->order.getProducts().stream().mapToDouble(Product::getPrice).sum())).forEach((id,totalPrice)-> System.out.println(id + ":" + totalPrice));
        //Exercise 14 â€” Obtain a data map with list of product name by category ; COLLECTORS.MAPPING usage <-----------------------
        System.out.println("\n\nExercise 14 â€” Obtain a data map with list of product name by category");
        products.stream().collect(Collectors.groupingBy(Product::getCategory,Collectors.mapping(Product::getName,Collectors.toList()))).forEach((category, names)-> System.out.println(category + " : " + String.join(",", names)));
        // Exercise 15 â€” Get the most expensive product by category
        System.out.println("\n\nExercise 15 â€” Get the most expensive product by category");
        products.stream().collect(Collectors.groupingBy(Product::getCategory,Collectors.maxBy(Comparator.comparing(Product::getPrice)))).forEach(((category,maxPrice)-> System.out.println(category + "=>"+maxPrice)));

        // Exercise 16 â€” Get the most expensive product price in each category
        System.out.println("\n\nExercise 16 â€” Get the most expensive product by category");
        // TODO:
